...
from app.modules.auth.router import get_current_admin, get_current_employee, get_current_user
from app.modules.files.qdrant_client import QdrantVectorStore
from app.modules.files.schemas import FileListResponse, FileUploadResponse
...
def _get_file_service(db: Session = Depends(get_db)) -> FileService:
    ...
    return FileService(db, storage, vector_store, embedding_provider)
...
@router.post(
    ...
    summary="Загрузка административного файла",
)
def upload_admin_file(
    ...
    return service.upload_admin_file(user, file)
...
def upload_customer_file(
    ...
    return service.upload_customer_file(user, customer_id, file)


@router.get("/admin", response_model=FileListResponse, summary="Список административных файлов")
